...
<script>
	$(document).ready(function() {
		...
		$("button.pictureBtn").on("click", function() {
			var picName = $(this).val();
			$("#pictureModal img").attr("src", "${pageContext.request.contextPath}/resources/pictures/"+picName);
			$("#pictureModal").modal();
		});

		$("button.pictureFormBtn").on("click", function() {
			var eid = $(this).val();
			var picName = $(this).parents("tr").find("button.pictureBtn").val();
			console.log(picName);
			$("#pictureFormModal input#picName").val(picName);
			$("#pictureFormModal input#eid").val(eid);
			$("#pictureFormModal").modal();
		});
				...
				<div class="panel-body">
					...
					<table class="table">
						<thead>
							<tr>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=eid">사번</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=name_">이름/사진</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=ssn">주민번호</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=phone">전화번호</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=hiredate">입사일</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=reg_name">지역명</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=dept_name">부서명</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=job_title">직위명</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=basicpay">기본급</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=extrapay">수당</a></th>
								<th><a
									href="${pageContext.request.contextPath}/employee/list?key=pay">급여</a></th>
								<th>사진등록</th>
								<th>삭제/수정</th>
							</tr>
						</thead>
						<tbody>
							...
							<c:forEach var="e" items="${list}">
								<tr>
									<td>${e.eid}</td>
									<td>${e.name_}/
										<button type="button" class="btn btn-default btn-xs pictureBtn"
									...
									<td>${e.dept_name}</td>
									<td>${e.job_title}</td>
									<td><fmt:formatNumber value="${e.basicpay}"
											groupingUsed="true" /></td>
									<td><fmt:formatNumber value="${e.extrapay}"
											groupingUsed="true" /></td>
									<td><fmt:formatNumber value="${e.pay}" groupingUsed="true" /></td>
									<td><button type="button" class="btn btn-default btn-xs pictureFormBtn" value="${e.eid}">사진등록</button></td>
									<td><button type="button" class="btn btn-default btn-xs btnDelete" value="${e.eid}">삭제</button>/
										<button type="button"
											class="btn btn-default btn-xs  btnUpdateForm" value="${e.eid}">수정</button></td>
								</tr>
							</c:forEach>
		...
		<div id="pictureFormModal" class="modal fade" role="dialog">
			<div class="modal-dialog modal-md">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form action="${pageContext.request.contextPath}/employee/fileUpload" method="POST" enctype="multipart/form-data" class="form-inline">
