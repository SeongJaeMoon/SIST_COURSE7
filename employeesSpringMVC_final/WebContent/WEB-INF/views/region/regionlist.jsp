...
<script>
	$(document).ready(function() {
		...
		$("button.btnUpdate").on("click", function() {
			...
			//입력폼에 대한 hidden 태그의 value 속성에 기존 reg_id 설정
			//입력폼에 대한 기존 reg_name 설정
			$("div#input .panel-heading").text("지역 수정");
			$("div#input form").attr("action", "${pageContext.request.contextPath}/region/update");
			$("div#input input[type='hidden']").attr("value", obj.reg_id);
			$("div#input input#reg_name").attr("value", obj.reg_name);
					...
					<tbody>
						...
						<tr>
						 ...
						 <tr>
							...
							<td>${e.reg_name}</td>
							<td><button type="button"
									class="btn btn-default btn-xs  btnDelete" ${(e.deleteCheck == 0)?"":"disabled"} value="${e.reg_id}">삭제</button></td>
							<td><button type="button"
